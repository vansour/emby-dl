...
- **尾部连接错误误判下载失败**：数据全部接收完毕后的流关闭错误（`error decoding response body`）不再导致下载失败，文件大小已达预期时直接视为完成
- **续传速度虚高**：`set_position` 跳位导致进度条瞬时 932 MiB/s，改为从 0 开始计速，消息栏显示 `(+9.50 GiB 续传)`

## v0.0.10 (2026-07-01)

### Bug 修复

- **下载失败自动重试**：网络偶发中断后不再直接放弃，自动重试最多 3 次，利用断点续传从中断处继续

## v0.0.9 (2026-07-01)

### Bug 修复

- **断点续传 offset 校验误报**：206 响应中 `Content-Length` 是剩余字节数，校验逻辑误将其当作完整文件大小，导致已下载部分超过剩余大小时报错退出

## v0.0.8 (2026-07-01)

### Bug 修复

- **大文件下载 1 小时超时**：`reqwest` 全局超时设为 3600s，导致超过 1 小时的大文件强制中断（报 `error decoding response body`），已移除总超时，仅保留连接超时
